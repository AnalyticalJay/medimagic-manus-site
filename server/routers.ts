  ...
  onlineConsultation: router({
    submit: publicProcedure
      .input(z.object({
        ...
        preferredAdministration: z.string().min(1),
        currentProblem: z.string().min(1),
        currentProblemOther: z.string().optional(),
        servicesRequired: z.string().min(1),
        servicesRequiredOther: z.string().optional(),
        historyOfProblem: z.string().min(1),
        historyOfProblemOther: z.string().optional(),
        paymentOption: z.string().min(1),
        paymentOptionOther: z.string().optional(),
        appointmentCancellation: z.boolean(),
        telephoneConsultation: z.boolean(),
        homeVisits: z.boolean(),
        popiaAgreement: z.boolean(),
        urgencyLevel: z.string().min(1),
        urgencyDescription: z.string().min(1),
        availability: z.string().min(1),
        availabilitySpecific: z.string().optional(),
        consultationMethod: z.string().min(1),
      }))
      .mutation(async ({ input }) => {
        const result = await createOnlineConsultationSubmission({
          ...
          specificService: input.servicesRequired,
          briefDescription: input.urgencyDescription,
          urgency: "medium" as any,
          preferredContactMethod: input.preferredCommunication as any,
          status: "pending",
        });
        ...
        const submission = {
          ...
          briefDescription: input.urgencyDescription,
          urgency: "medium",
          preferredContactMethod: input.preferredCommunication,
        };
